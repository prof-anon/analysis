#Based on https://github.com/amiller/mev-meditations
...
prof_scenario = "prof"
all_scenarios = [prof_share_scenario, prof_scenario, mev_share_scenario]
scenario_labels = {
    prof_share_scenario : "PROF-Share",
    prof_scenario : "PROF",
    mev_share_scenario : "MEV-Share",
}
...
class Experiment():
    ...
    def __init__(self, param_type=random_type, direction_type=diff_type, num_iters=10, users_range=list(range(2,101)), arbitrage_type=cexdex_arbitrage, params=None, file_dir="data/test/",debug=False, extra_params={max_demand_scale_type:4, max_demand_batch_type: 5}):
    ...
    def gen_preferences(self, num_users, params=None, param_type=None, direction_type=None):
        ...
        elif direction_type not in [same_type, diff_type]:
    ...
    def setup_accounts(self, scenario, num_users):
        ...
        Args:
            ...
            num_users (int): number of users in the block
        Returns:
            tuple of dictionaries with {"entity_name": Account object} for users and arbitragers
        """
        user_accounts = {}
        arbi_accounts = {}
    ...
    def get_pools(self, user_accounts, arbi_accounts):
        ...
        if self.arbitrage_type == cexdex_arbitrage:
            ...
            #only arbitragers use CEX
            chain2 = Chain(static=1.0, accounts=arbi_accounts, chainid=f"chain2")
        elif self.arbitrage_type == dexdex_arbitrage:
            chain1 = Chain(poolA=self.pool_liquidity, poolB=self.pool_liquidity, accounts=accounts, chainid=f"chain1")
            #only arbitragers use 2nd chain
    ...
    def get_user_util_old(self, pref):
        if self.param_type == random_type:
            # for random scenario we care about the real utility calculation
            user_tokens = [self.user_portfolio, self.user_portfolio]
            return utility(pref, user_tokens)
        else:
            # for all other scenario we only care how many tokens were gained
            return 0
    ...
    def run_iteration(self, scenario, num_users, iter_num, preferences, param, param_type="", all_stats=False):
        if not self.quiet:
            print(f"-----------scenario: {scenario} | total users: {num_users} | {param_type} {param}: ({iter_num})-----------")
    ...
    def run_all(self, save_state=True, all_stats=False):
        ...
        for iter_num in range(self.num_iters):
            for user_index in range(len(self.users_range)):
                ...
                preferences = self.gen_preferences(num_users, param_type=self.param_type)    
    ...
    def parse_data(self, stat_name=user_util_stat, by="param", merge=True):
        ...
        if merge:
            ...
            for scenario in all_scenarios:
                ...
                parsed_data[scenario]["cis"] = confidence_interval(parsed_data[scenario]["averages"], parsed_data[scenario]["stds"], self.num_iters, alpha=.05, z=1.624)
        else:
            ...
            for nb in not_by_group:
                ...
                for scenario in all_scenarios:
                    ...
                    parsed_data[scenario][nb]["cis"] = confidence_interval(parsed_data[scenario][nb]["averages"], parsed_data[scenario][nb]["stds"] , self.num_iters, alpha=.05, z=1.624)
    ...
    def plot_stats_line(self, all_stats=False, stat_names=None, by="param", merge=True):
        assert by in ["num_users", "param"]
        stat_filenames = {}
        if stat_names is not None:
            for n in stat_names:
        ...
        for n in stat_filenames:
            ...
            if merge:
                ...
                if np.isclose(range_y, 0):
                    range_y = max_y*.5
                ax.set_ylim(min_y-(range_y/4), max_y+(range_y/4))
                plt.legend()
                plt.title(f'{stat_data["nb_range"]} {nb_name} {self.arbitrage_type} arbitrage, {self.param_type}\n{self.num_iters} iterations 95% confidence')
                ...
                for nb in stat_data[all_scenarios[0]].keys():
                    ...
                    ax.set_ylim(min_y-(range_y/4), max_y+(range_y/4))
                    plt.legend()
                    plt.title(f'{nb} {nb_name} {self.arbitrage_type} arbitrage, {self.param_type}\n{self.num_iters} iterations 95% confidence')
                    fig_filename = self.file_location + f"_{nb}{nb_name}_by{by}_line.png"
                    fig.savefig(fig_filename)
                    print(fig_filename)
            
    def plot_stats_bar(self, all_stats=False, stat_names=None, by="param", merge=True):
        assert by in ["num_users", "param"]
        stat_filenames = {}
        if stat_names is not None:
            for n in stat_names:
        ...
        for n in stat_filenames:
            ...
            if merge:
                ...
                for scenario in all_scenarios:
                    ...
                    cis = np.array(stat_data[scenario]["cis"])
                    min_y = min(min_y, min(averages-cis))
                    max_y = max(max_y, max(averages+cis))
                    offset = width*multiplier
                    multiplier+=1
                    ax.bar(xs + offset, averages, width, label=scenario, yerr=cis)
...
def make_paper_plots_bar(file_location='data/paper/', output_file="paper_plot_sim.png"):
    data_filename = os.path.join(file_location, "cexdex_max_demand_ratio_1000iters_20to100-81users_0.25to16-8params_user_util_param_merged.json")
    assert os.path.isfile(data_filename)
    stat_data = json.load(open(data_filename))
    fig, axs = plt.subplots(2, sharex=False, layout='constrained')
    fig.set_figheight(6)
    ...
    for i in range(2):
        ...
        j = 0
        for scenario in all_scenarios:
            if i == 1 and scenario == prof_scenario:
                j+=1
                continue
            if i == 0:
                offset = width * multiplier
            ...
            measurement = stat_data[scenario]["averages"][i*len(v):(i+1)*len(v)]
            # stds = stat_data[scenario]["stds"][i*len(v):(i+1)*len(v)]
            # rects = ax.bar(x + offset, measurement, width, label=scenario, yerr=stds, color=c[j])
            rects = ax.bar(x + offset, measurement, width, label=scenario_labels[scenario], color=c[j])
            j+=1
            multiplier += 1
    ...
    fig.suptitle('''Figure 5: Average User Utility for Demand Ratio of 0.25, 0.50, 0.75, and 
1, 2, 4, 8 for 20 to 100 users/block over 1,000 iterations. Error bars 
indicate the  standard deviation. We do not plot the average utility 
for PROF for higher Demand Ratios, as it becomes negligible in 
comparison to redistributive mechanisms.''')
    fig.legend([scenario_labels[s] for s in all_scenarios], ncols=3,loc=(.4,.75))
